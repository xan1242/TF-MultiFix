...
int lEhScript_ModuleRead_FinishCB_Hook(uintptr_t unk1, uintptr_t unk2)
{
    ...
    strcpy(modNameLower, modName);
    str_tolower(modNameLower);

    if (strstr(modNameLower, "psmf"))
    {
    ...
    if (strstr(modNameLower, "title.p"))
    {
        SetGameState(EHSTATE_TITLE);
    }

    if (strstr(modNameLower, "deck.p"))
    {
        uintptr_t bs = 0;
        uintptr_t sz = 0;
    ...
    if (strstr(modNameLower, "duelmgr.p"))
    {
        uintptr_t bs = 0;
        uintptr_t sz = 0;
...
int lEhModule_Load_EndCallback_Hook(uintptr_t unk1, uintptr_t unk2)
{
    ...
    if (strstr(modNameLower, "dueldraw.p"))
    {
        uintptr_t bs = 0;
        uintptr_t sz = 0;
    ...
    if (strstr(modNameLower, "dueleng.p"))
    {
        uintptr_t bs = 0;
        uintptr_t sz = 0;
...
uintptr_t HookCallback(uintptr_t addr, uintptr_t dest)
{
    ...
    uintptr_t cbptr = MIPS_DiscoverPtr_a2(addr + base_addr, &ptrLUI, &ptrADDIU);

    uint32_t newLUI = 0;
    uint32_t newADDIU = 0;

    MIPS_CalcNewA2Value((uint32_t)dest, &newLUI, &newADDIU);
...
void TFFixesInject()
{
    ...
    injector.MakeCALL(0x5BF0, (uintptr_t)&lSoftReset_Hook);

    uintptr_t modreadcb = HookCallback(0x3D5B0, (uintptr_t)&lEhScript_ModuleRead_FinishCB_Hook);
    ptr_lEhScript_ModuleRead_FinishCB = (int(*)(uintptr_t, uintptr_t))modreadcb;


    uintptr_t modloadcb = HookCallback(0x39CA4, (uintptr_t)&lEhModule_Load_EndCallback_Hook);
    ...
    // force non-taboo
    injector.MakeJMPwNOP(0x2068, 0x21AC + base_addr);

    // fix line spacing
    injector.MakeNOP(0x17B8);
    injector.WriteMemory32(0x17A8, 0x00A02021); // move a0, a1
    injector.WriteMemory16(0x1430, LINE_SPACING_VAL);

    injector.WriteMemory16(0x2848, LINE_SPACING_VAL);
    injector.WriteMemory16(0x28F8, LINE_SPACING_VAL);
    injector.WriteMemory16(0x292C, LINE_SPACING_VAL);


    injector.WriteMemory16(0x19BC, (uint16_t)(-LINE_SPACING_VAL));
    injector.WriteMemory16(0x1F70, (uint16_t)(-LINE_SPACING_VAL));
    injector.WriteMemory16(0x37B8, (uint16_t)(-LINE_SPACING_VAL));
    injector.WriteMemory16(0x386C, (uint16_t)(-LINE_SPACING_VAL));

    injector.WriteMemory16(0x53CC, (uint16_t)(-LINE_SPACING_VAL));

    injector.WriteMemory32(0x5AE8, 0x34050001); // li a1, 1
    injector.MakeCALL(0xE6A8, (uintptr_t)&YgSys_GetLang_Hook);
    injector.MakeCALL(0x16344, (uintptr_t)&YgSys_GetLang_Hook);
    injector.MakeCALL(0x16DF0, (uintptr_t)&YgSys_GetLang_Hook);
    injector.MakeCALL(0x16E3C, (uintptr_t)&YgSys_GetLang_Hook);
    injector.MakeCALL(0x17DF4, (uintptr_t)&YgSys_GetLang_Hook);
    injector.MakeCALL(0xC3B0, (uintptr_t)&YgSys_GetLang_Hook);
    injector.WriteMemory16(0x17E64, 0x480);

#ifndef __INTELLISENSE__
    // global font Y offset
    injector.MakeCALL(0x1844, (uintptr_t)&hkPrintLineFontYOffset);
    injector.MakeCALL(0x34D8, (uintptr_t)&hkPrintBoxFontYOffset);
#endif

    if (mfconfig_GetSwapConfirmButtons())
    {
        // swap cross and circle
        injector.MakeJMPwNOP(0x8DA0, (uintptr_t)&YgSys_GetAssignButton_Hook);
    }
